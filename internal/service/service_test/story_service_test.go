...
func TestCreateStory_Success(t *testing.T) {
	...
	newLimit.Count += 2
	ddb.EXPECT().AddDailyLimit(gomock.Any(), tx, newLimit).Return(nil)
...
func TestUserChoiceCharacters_Success(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	setcache := mock_service.NewMockSettingCache(ctrl)
	subdb := mock_service.NewMockSubscriptionDatabase(ctrl)
	...
	variant := models.NewStoryVariant(1, "characters", data)
	vardb.EXPECT().GetActiveVariants(gomock.Any(), userID).Return([]*models.StoryVariant{
		variant,
	}, nil)
	dcache.EXPECT().CheckExceededLimit(gomock.Any(), userID).Return(false, nil)
	ddb.EXPECT().GetDailyLimit(gomock.Any(), userID).Return(nil, nil)
	...
	setcache.EXPECT().GetSetting(gomock.Any(), models.SettingKeyLimitBaseDay).Return("20", nil)
	newLimit := models.NewDailyLimit(userID, 0, 20)
	storysegments := make([]*models.StoryMessage, 0)
	msgdb.EXPECT().GetAllStorySegments(gomock.Any(), 1).Return(storysegments, nil)
	stai.EXPECT().GenerateNextStorySegment(gomock.Any(), gomock.Any()).Return(&models.StoryNode{
		Narrative:      "something",
		ShortNarrative: "something",
		Choices:        []string{"something"},
		IsEnding:       false,
	}, nil)
	var tx pgx.Tx
	tman.EXPECT().BeginTx(gomock.Any()).Return(tx, nil)
	msgdb.EXPECT().AddStoryMessages(gomock.Any(), tx, gomock.Any()).Return(nil)
	choicesData, _ := json.Marshal([]string{"something"})
	addingVariant := models.NewStoryVariant(1, "actions", choicesData)
	vardb.EXPECT().UpdateVariant(gomock.Any(), tx, addingVariant).Return(nil)
	newLimit.Count += 1
	ddb.EXPECT().AddDailyLimit(gomock.Any(), tx, newLimit).Return(nil)
	tman.EXPECT().CommitTx(gomock.Any(), tx).Return(nil)
	response, err := serv.UserChoice(ctx, userID, "2")
	require.Nil(t, err)
	expectedHero := &models.Hero{}
	msg := text_messages.CreateHeroMessage(expectedHero)
	resp := text_messages.TextNarrativeWithChoices("something", []string{"something"})
	require.Equal(t, []string{msg, resp}, response)
}
func TestUserChoiceActions_Success(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	variant := models.NewStoryVariant(1, "actions", choicesData)
	...
	stai.EXPECT().GenerateNextStorySegment(gomock.Any(), gomock.Any()).Return(&models.StoryNode{
		Narrative:      "Продолжение истории...",
		ShortNarrative: "Краткое продолжение...",
		Choices:        []string{"новый выбор 1", "новый выбор 2"},
		IsEnding:       false,
	}, nil)
	...
	newChoicesData, _ := json.Marshal([]string{"новый выбор 1", "новый выбор 2"})
	addingVariant := models.NewStoryVariant(1, "actions", newChoicesData)
	vardb.EXPECT().UpdateVariant(gomock.Any(), tx, addingVariant).Return(nil)
	...
	userVariant := models.Extension{Narrative: "choice2"}
	expectedMsg := text_messages.CreateExtensionMessage(&userVariant)
	expectedResp := text_messages.TextNarrativeWithChoices("Продолжение истории...", []string{"новый выбор 1", "новый выбор 2"})

	require.Equal(t, []string{expectedMsg, expectedResp}, response)
}
